...
void init_game_state(GameState * game, int width, int height){
    ...
    while(player_paths[game->num_players] != NULL){
        if(strlen(player_paths[game->num_players]) > 16){
            perror("player name too long (only 16 chars)");
            exit(EXIT_FAILURE);
        }
...
int handle_moves(GameState *game, struct timeval *timeout, unsigned char * player_moves, fd_set * read_fd, unsigned int k) {
    int ready = select(highest_fd, read_fd, NULL, NULL, timeout);
    ...
    player_moves[k] = move;
    process_player_move(game, k, player_moves[k]);
